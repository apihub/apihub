...
func (s *S) TestUnsubscribePlugin(c *C) {
	...
	payload := `{"service":"` + service.Subdomain + `"}`
	b := strings.NewReader(payload)

	s.router.Delete("/api/plugins/:name/subscriptions", s.Api.route(pluginsHandler, "UnsubscribePlugin"))
	req, _ := http.NewRequest("DELETE", "/api/plugins/"+conf.Name+"/subscriptions", b)
	req.Header.Set("Content-Type", "application/json")
	s.env[CurrentUser] = owner
	webC := web.C{Env: s.env}
	s.router.ServeHTTPC(webC, s.recorder, req)

	expected := `{"name":"` + conf.Name + `","service":"` + service.Subdomain + `"}`
	c.Assert(s.recorder.Header().Get("Content-Type"), Equals, "application/json")
	c.Assert(s.recorder.Body.String(), Equals, expected)
	c.Assert(s.recorder.Code, Equals, http.StatusOK)
}

func (s *S) TestUnsubscribePluginWhenServiceDoesNotExist(c *C) {
	owner.Save()
	defer owner.Delete()
	...
	s.router.Delete("/api/plugins/:name/subscriptions", s.Api.route(pluginsHandler, "UnsubscribePlugin"))
	req, _ := http.NewRequest("DELETE", "/api/plugins/a/subscriptions", b)
	req.Header.Set("Content-Type", "application/json")
	s.env[CurrentUser] = owner
	webC := web.C{Env: s.env}
	s.router.ServeHTTPC(webC, s.recorder, req)
