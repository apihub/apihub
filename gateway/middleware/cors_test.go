...
func (s *S) TestCors(c *C) {
	...
	cors := &Cors{}
	cors.AllowedOrigins = []string{"http://apihub.example.org"}
	...
	req, _ := http.NewRequest("OPTIONS", "http://apihub.example.org", nil)
	req.Header.Add("Origin", "http://apihub.example.org")
	req.Header.Add("Access-Control-Request-Method", "GET")
	req.Header.Add("Access-Control-Request-Headers", "origin")

	cors.ProcessRequest(res, req, next)
	c.Assert(res.Header().Get("Access-Control-Allow-Origin"), Equals, "http://apihub.example.org")
